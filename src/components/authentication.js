...
function Authentication(props) {
  ...
  const createAccount = async () => {
    ...
    try {
      await auth._createUserWithEmailAndPassword(auth, username, password);
      // promises cho 2 hàm async: tạo tài khoản với auth và lưu thông tin khác của tài khoản vào fs_db
    } catch (error) {
  ...
  const signIn = async () => {
    ...
    setErrorMessage(null);
    try {
      await auth._signInWithEmailAndPassword(auth, username, password);
      // promises cho 2 hàm async: đăng nhập và tải thông tin tài khoản về
    } catch (error) {
  ...
  if (user) {
    contents = (
      <>
        <p>Welcome back {user.email}! You can log out below.</p>
